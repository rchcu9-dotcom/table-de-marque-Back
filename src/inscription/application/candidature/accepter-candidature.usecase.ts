...
@Injectable()
export class AccepterCandidatureUseCase {
  ...
  async execute(id: number): Promise<{ id: number; statut: string }> {
    ...
    if (inscription.statut !== InscriptionStatut.CANDIDATE) {
      throw new BadRequestException(
        'Seule une candidature en statut CANDIDATE peut être acceptée',
      );
    }

    // Vérifier le nombre max d'équipes réservées
    ...
    // CANDIDATE → PAIEMENT_ATTENDU (+ RESERVEE implicitement via le statut)
