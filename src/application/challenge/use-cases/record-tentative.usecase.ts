...
import { TentativeAtelier, TentativeMetrics } from '@/domain/challenge/entities/tentative-atelier.entity';
...
export class RecordTentativeUseCase {
  ...
  async execute(params: { atelierId: string; joueurId: string; metrics: TentativeMetrics }): Promise<TentativeAtelier> {
    ...
    if (atelier.type !== params.metrics.type) {
      throw new Error(`Metrics type ${params.metrics.type} does not match atelier type ${atelier.type}`);
    }
