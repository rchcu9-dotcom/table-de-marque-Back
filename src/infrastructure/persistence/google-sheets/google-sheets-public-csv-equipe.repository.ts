  ...
  implements EquipeRepository
{
  ...
  async findAllEquipes(): Promise<Equipe[]> {
    ...
    const all = Object.values(poules).flatMap((poule) => poule.equipes);
    const dedup = new Map<string, Equipe>();
    all.forEach((eq) => {
      const id = this.normalizeTeamKey(eq.name);
      const logo = logos[id] ?? eq.logoUrl ?? null;
      dedup.set(id, {
        ...eq,
        id,
        logoUrl: logo,
  ...
  async findEquipeById(id: string): Promise<Equipe | null> {
    const target = this.normalizeTeamKey(id);
    const all = await this.findAllEquipes();
    return all.find((eq) => this.normalizeTeamKey(eq.id) === target) ?? null;
  }
