  ...
  async findAll(): Promise<Match[]> {
    ...
    const enriched = filteredMatches.map((row) => {
      ...
      return new Match(
        ...
        scoreA,
        scoreB,
        buildTeamLogoUrl((row.EQUIPE_ID1 ? equipeById.get(row.EQUIPE_ID1)?.EQUIPE : null) ?? row.EQUIPE1),
        buildTeamLogoUrl((row.EQUIPE_ID2 ? equipeById.get(row.EQUIPE_ID2)?.EQUIPE : null) ?? row.EQUIPE2),
        pouleCode,
        pouleName,
        competitionType,
        surface,
